...
2. Let the model iteratively call the native `search_issues(query, limit, search_type)` and `get_issue(issue_number)` tools, including multiple calls in one step when useful.
